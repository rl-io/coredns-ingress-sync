...
type Config struct {
	...
	// ServiceAnnotationsEnabled gates annotation-driven Service support
	// entirely, separate from the hostname-annotation-presence check on any
	// given Service. False (default) means no RBAC, watches, or cache
	// scoping touch corev1.Service beyond what's already needed for other
	// checks -- watching all cluster Services is a materially bigger
	// footprint than any CRD watch, so this needs an explicit opt-in.
	ServiceAnnotationsEnabled bool
	// ServiceHostnameAnnotationKey is the annotation whose presence (and
	// value) makes a Service a hostname candidate; its mere presence is the
	// participation signal, mirroring ingressClassName/gatewayClassName/
	// IngressRoute-presence on the other three sources.
	ServiceHostnameAnnotationKey string
	ExcludeServices              string // Comma-separated list of Service names or namespace/name
	// ClusterDomain is used only to build the Service source's synthesized
	// target CNAME (<name>.<namespace>.svc.<ClusterDomain>.), since -- unlike
	// the other three sources -- there's no operator-supplied external
	// target CNAME to configure per Service.
...
func Load() *Config {
	...
	return &Config{
		...
		ServiceAnnotationsEnabled:    getEnvOrDefault("SERVICE_ANNOTATIONS_ENABLED", "false") == "true",
		ServiceHostnameAnnotationKey: getEnvOrDefault("SERVICE_HOSTNAME_ANNOTATION_KEY", "coredns-ingress-sync-hostname"),
		ExcludeServices:              getEnvOrDefault("EXCLUDE_SERVICES", ""),
